...
$Approvedstatus = $_POST["Approvedstatus"];
  ...
  $CandidateMDAprroveQry = "Update indsys1027employeepayrolldeduction set  Editrequestapproval ='$Approvedstatus', Addormodifydatetime ='$date'
  ...
  $resultCandidateMDAprroveQry = $conn->query($CandidateMDAprroveQry);
    ...
    while($row = mysqli_fetch_array($result_Chapter)) {  
      $Editrequestreasonraisedby =$row['Editrequestreasonraisedby'];
    
     
      } 
    }

    $GetUsermailid = "SELECT * FROM indsys1000useradmin where  Userid = '$Editrequestreasonraisedby'  AND Clientid='$Clientid'  ORDER BY Userid";
