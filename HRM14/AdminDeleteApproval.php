...
$Approvedstatus = $_POST["Approvedstatus"];
  ...
  $SalMonth = $_GET['SalMonth'];
  $Salyear = $_GET['Salyear'];


  $CandidateMDAprroveQry = "Update indsys1027employeepayrolldeduction set  Deleterequestapproval ='$Approvedstatus', Addormodifydatetime ='$date'
  WHERE Employeeid = '$Employeeid' and Clientid ='$Clientid' and Salyear='$Salyear' and SalMonth='$SalMonth' ";  
  $resultCandidateMDAprroveQry = $conn->query($CandidateMDAprroveQry);

    if($Approvedstatus =="Approved")
    {
        $Deleterequestreasonraisedby="";
      
    $GetChapter = "SELECT * FROM indsys1027employeepayrolldeduction where  Employeeid = '$Employeeid' and Salyear='$Salyear' AND SalMonth='$SalMonth' AND Clientid='$Clientid'  ORDER BY Employeeid";
    $result_Chapter = $conn->query($GetChapter );
    if(mysqli_num_rows($result_Chapter) > 0) { 


    while($row = mysqli_fetch_array($result_Chapter)) {  
      $Deleterequestreasonraisedby =$row['Deleterequestreasonraisedby'];
    
     
      } 
    }

    $GetUsermailid = "SELECT * FROM indsys1000useradmin where  Userid = '$Deleterequestreasonraisedby'  AND Clientid='$Clientid'  ORDER BY Userid";
    $result_Usermail = $conn->query($GetUsermailid );
    if(mysqli_num_rows($result_Usermail) > 0) { 


    while($row = mysqli_fetch_array($result_Usermail)) {  
      $EditingUserMail =$row['Emailid'];
                    ...
                    <td class='wrapper'>
                      <table role='presentation' border='0' cellpadding='0' cellspacing='0'>
                        <tr>
                          ...
                          <p>Dear User,</p>
                          <p> Your request has been approved. For Payroll Deduction
