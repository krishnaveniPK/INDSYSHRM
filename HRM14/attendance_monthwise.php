...
// $query = "SELECT * FROM indsys1017employeemaster where  (EmpActive='Active' or  (EmpActive='Deactive' AND YEAR(Leftdate)='$Year' AND MONTH(Leftdate)<='$nmonth' ) )  AND Clientid='$Clientid' AND Type_Of_Posistion in(". $Category.")  ORDER BY Employeeid";

$query = "SELECT * FROM indsys1017employeemaster where EmpActive IN('Active','Deactive') AND (DATE(Leftdate) >'$fdaymonth'   OR Leftdate IS NULL)  AND Clientid='$Clientid' AND Type_Of_Posistion in(". $Category.")  ORDER BY Employeeid";
...
//     $i ++;
// }
// $query = $query . " WHERE Type_Of_Posistion in (" . $selectedOption . ") and EmpActive='Active' ";
//echo $query;
...
foreach ($emp_id as $row)
 {
   $overAllLopMin=0;
    ...
    $sql_perform_att = " SELECT * FROM indsys1030empdailyattendancedetail WHERE Employeeid = '" . $emp_id . "'";
...
<?php
    ...
    $sql = "SELECT  Count(AttenStatus) from vwattendenceclosestatus where Attendencedate>='$fdaymonth' and Attendencedate <='$ldaymonth' and Employeeid = '$emp_id' and AttenStatus='P' and Empattendencestatus='Close'";
    ...
    $sqlHD = "SELECT  Count(AttenStatus) from vwattendenceclosestatus where Attendencedate>='$fdaymonth' and Attendencedate <='$ldaymonth' and Employeeid = '$emp_id'    and AttenStatus='HD' and Empattendencestatus='Close'";
    ...
    $sqlOD = "SELECT  Count(AttenStatus) from vwattendenceclosestatus where Attendencedate>='$fdaymonth' and Attendencedate <='$ldaymonth' and Employeeid = '$emp_id'    and AttenStatus='OD' and Empattendencestatus='Close' ";
    ...
    $sqlworkinghrs = "SELECT SUM(HOUR(REPLACE(Workinghours, '.', ':'))*60+MINUTE(REPLACE(Workinghours, '.', ':'))) as WorkinghoursHRM from vwattendenceclosestatus where Attendencedate>='$fdaymonth' and Attendencedate <='$ldaymonth' and Employeeid = '$emp_id'     and Empattendencestatus='Close' ";
...
if(empty($TotalWorkingHRS))
{
    ...
    $result = mysqli_query($conn, "select Count(*) as total from vwholidaymaster where Monthname ='$month' and Year = '$year' and Dayname!='Sunday' AND Clientid ='1'");
...
//     }
   ...
   // echo "sdcfsdf  $Workeddays $Leavedays";exit;
    ...
    $result = get_attendance($conn, $emp_id, $periods, $date_of_joining);
    ...
    foreach ($result as $row) {
        ...
        $sqlLOP = "SELECT  SUM(HOUR(REPLACE(Lophrs, '.', ':'))*60+MINUTE(REPLACE(Lophrs, '.', ':'))) as LophrsHRM from vwattendenceclosestatus where Attendencedate>='$fdaymonth' and Attendencedate <='$ldaymonth' and Employeeid = '$emp_id'     and Empattendencestatus='Close'";
...
?>
   ...
   ?></td>
       ...
       <td><?php {
          ...
          $sql_weekoff = "SELECT Week_Off FROM indsys1017employeemaster WHERE Employeeid = '$emp_id'";
            // echo $sql_weekoff;
