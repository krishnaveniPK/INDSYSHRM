...
try
{
    ...
    $Employeeid =$form_data['Employeeid'];
     $Sno = 0;
        $resultExistsnew = "SELECT Nextno FROM vwemployeeeditingrightsnextno WHERE Employeeid = '$Employeeid' AND Clientid = '$Clientid' LIMIT 1";
        $resultExists01new = $conn->query($resultExistsnew);
        if (mysqli_fetch_row($resultExists01new))
        {

            $GetChapter = "SELECT * FROM vwemployeeeditingrightsnextno WHERE Employeeid = '$Employeeid' AND Clientid = '$Clientid'  ";
...
$Randompassword=rand(0000,9999);
  ...
  $resultExists = "SELECT Employeeid FROM indsys1020employeeeditingrights WHERE Employeeid = '$Employeeid' AND Sno='$Vaccinatedsno'AND Clientid = '$Clientid'  LIMIT 1";
  $resultExists01 = $conn->query($resultExists);

  if (mysqli_fetch_row($resultExists01))
  {
  $resultExists = "Update indsys1020employeeeditingrights set 
  ...
  Addormodifydatetime ='$date'
     ...
     WHERE Employeeid = '$Employeeid' and Sno ='$EditingRightSno' ";
  $resultExists01 = $conn->query($resultExists);
  ...
  {
        $sqlsave = "INSERT IGNORE INTO indsys1020employeeeditingrights (Clientid,Employeeid, Sno,MD_Approval_Status,Editingreason,EditingUserid,EditingUsername,Randomuserid,Randompassword,EditingStatus,Addormodifydatetime,EditingUserMail)
        VALUES ('$Clientid','$Employeeid','$EditingRightSno','Pending','$EditingReason','$user_id','$username','$Randomuserid','$Randompassword','Open','$date','$usermail')";
              ...
              <tr>
                <td class='wrapper'>
                  <table role='presentation' border='0' cellpadding='0' cellspacing='0'>
                    ...
                    <tr>
                    <td>Click<a target='_blank' href='$domain/HRM10/EditApprovedMD.php?Employeeid=$Employeeid&Sno=$EditingRightSno'> Here </a>For Approval</td>
                    </tr>
                  </table>
                </td>
              </tr>
...
$ToMail = $MailTo;
$SubjectMail = "Employee Master and Detail Editing Approval";
...
if($MethodGet == 'LoginUpdate')
{
  ...
  $resultExists = "SELECT Employeeid FROM indsys1020employeeeditingrights WHERE Employeeid = '$Employeeid' AND Clientid = '$Clientid' AND Randomuserid ='$EditingUserid' AND  Randompassword ='$EditingPassword' AND EditingStatus='Open'  LIMIT 1";
  $resultExists01 = $conn->query($resultExists);
...
if($MethodGet == 'LoginDestroy')
{
  ...
  EditingStatus ='Close'
     ...
     WHERE Employeeid = '$Employeeid' And Randomuserid='$EditingUserid' And Randompassword='$EditingPassword' ";
  $resultExists01 = $conn->query($resultExists);
  $Message ="st";
