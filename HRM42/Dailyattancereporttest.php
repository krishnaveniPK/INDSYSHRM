...
//$attendancePdf = $pdf->Output('attendancePdf.pdf', 'I'  );

$attendancePdf = $pdf->Output('attendancePdf.pdf', 'S'  );
...
<body>
    Dear sir/madam,
    <p style="margin:13%">PFA for actual & audit attendance report of the day '.date("d-m-Y",strtotime($Attendancedate)).'</p>
...
$mail->AddStringAttachment($attendancePdf,'Actual_and_audit_attendance_report_'.date("d-m-Y",strtotime($Attendancedate)).'.pdf', 'base64',  'application/pdf');
if($mail->Send()){
	echo "Mail Sent";
}else{
	echo "Not Sent";
...

{
    ...
    $html="";
    $html .='<table border="1" >';
    $html .=' <thead><tr ><th colspan="18" ><h2 style="text-align:center">SAINMARKS INDUSTRIES(INDIA) PVT LTD-Tirupur</h2></th></tr>
    <tr>
    <th>Date</th>
    <th colspan="2">'.$Attendancedate2.'</th>
    <th colspan="2">No Of Present</th>
    <th >'.$NoofPresent.'</th>
    <th colspan="2">No Of Leave</th>
    <th >'.$Noofleave.'</th>
    <th colspan="2">No Of Absent</th>
    <th >'.$NoofAbsents.'</th>
    <th colspan="2">No Of Employee</th>
    <th >'.$NoofEmployee.'</th>
   ...
   ';
    ...
    <tr>
    <th colspan="4" style="text-align:center;width:32%"> Employee Information </th>
   
    <th colspan="7" style="text-align:center;width:34%"> Actual Attendance  </th>
    <th colspan="7" style="text-align:center;width:34%" > Audit Attendance  </th>
    </tr>
    <tr>
    ...
    <th style="width:3%"> LOP </th>
    <th style="width:6%"> OT Intime </th>
    <th style="width:7%">OT Outtime</th>
    
    <th  style="width:3%"> OT </th>
...
$Name ="$Title$Firstname$lastname";
   ...
   <td style="width:4%;text-align:center">'.$AttenStatus.'</td> 
   <td style="width:6%">'.$ActualIntime.'</td>
   <td style="width:6%">'.$ActualOuttime.'</td>
   <td style="width:3%">'.$Actualaudithrs.'</td>
   <td style="width:3%">'.$Auctualaditlop.'</td>
   <td style="width:6%">'.$ActualOTIntime.'</td>
   <td style="width:7%">'.$ActualOTOuttime.'</td>

   <td style="width:3%">'.$ActualOt_HRSNew.'</td>
   <td style="width:6%">'.$Intime.'</td>
   <td style="width:6%">'.$Outtime.'</td>
   <td style="width:3%">'.$Workinghours.'</td>
   <td style="width:3%">'.$Lophrs.'</td>
   <td style="width:6%">'.$OTIntime.'</td>
   <td style="width:7%">'.$OTOuttime.'</td>

   <td style="width:3%">'.$OT_HRS.'</td>
